...
class HandClassification(OnnxModel):
    ...
    def __call__(self, image, bboxes):
        ...
        predictions : np.ndarray
            Predictions from model (filtered to only peace, like, stop)
        """
        from utils.enums import GESTURE_FILTER
        ...
        # Filter to only keep peace (32), thumbs up (27), stop (35) gestures
        # Set all other gestures to None
        filtered_labels = []
        for label in labels:
            if label in GESTURE_FILTER:
                filtered_labels.append(label)
            else:
                filtered_labels.append(None)  # Ignore other gestures
